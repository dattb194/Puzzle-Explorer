...
public  class LineRendererSmoother : MonoBehaviour
{
    ...
    [HideInInspector] public Vector3[] InitialState = new Vector3[1];
    [HideInInspector] public float SmoothingLength = 2f;
    [HideInInspector] public int SmoothingSections = 10;

    public Material mat;

    public void GenerateMeshCollider()
    {
        GameObject obj = new GameObject("GenerateMeshCollider");
        ...
        TubeMesh tubeMesh = obj.AddComponent<TubeMesh>();
        var points = new List<Vector3>();
        for (int i = 0; i < Line.positionCount; i++)
        {
            points.Add(Line.GetPosition(i));
        }
        tubeMesh.GenerateTubeMesh(points, Line.widthMultiplier);
